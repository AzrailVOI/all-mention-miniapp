...
function renderChats(chats, infoMessage) {
    ...
    if (!chats || chats.length === 0) {
        let emptyContent = `
            <div class="empty-state">
                ...
                <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 12px;">Чаты не найдены</h3>
        ...
        if (infoMessage) {
            emptyContent += `
                <div class="info-box" style="margin-top: 20px; padding: 20px; background: var(--color-bg-elevated); border: 1px solid var(--color-border); text-align: left; font-size: 14px; line-height: 1.7;">
                    <strong style="display: block; margin-bottom: 12px;">ℹ️ Информация:</strong>
    ...
    chatsContainer.innerHTML = chats.map(chat => {
        ...
        const escapedTitle = window.escapeHtml ? window.escapeHtml(chatTitle) : chatTitle;
        return `
        <div class="chat-item" data-chat-id="${chat.id}" data-chat-title="${escapedTitle}">
            ...
            <div class="chat-info">
                <div class="chat-name">${window.escapeHtml ? window.escapeHtml(chat.title || 'Без названия') : (chat.title || 'Без названия')}</div>
